...
const fetchDatas = async (category) => {
  const { data } = await axios.get(`${BASE_URL}/${category}/${api_key2}`);
  return data;
};

const getSingleData = async (id) => {
  const { data } = await axios.get(`${BASE_URL}/Title/${api_key2}/${id}`);
  return data;
};

const getReviews = async (id) => {
  const { data: items } = await axios.get(
    `${BASE_URL}/Reviews/${api_key2}/${id}`
  );
  return items;
};
...
const autoCompleteSearch = async (input) => {
  const { data: results } = await axios.get(
    `${BASE_URL}/Search/${api_key2}/${input}`
  );
  return results;
};
...
const AdvancedSearch = async (genre) => {
  const { data: results } = await axios.get(
    `${BASE_URL}/AdvancedSearch/${api_key2}?genres=${genre}`
  );
  return results;
};
...
const FilterSearching = async (query) => {
  const { data } = await axios.get(
    `${BASE_URL}/AdvancedSearch/${api_key2}?${query}`
  );
  return data;
};
